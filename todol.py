...
def printToDo(j, checked=True):
    if checked == False:
        print(cl_number + printID(j) + cl_reset + cl_checkbox +
              "[ ]" + cl_reset + lines[j].replace("- [ ]", ""), end="")
    else:
        print(cl_number + printID(j) + cl_reset + cl_checkbox +
              "[X]" + cl_reset + lines[j].replace("- [X]", ""), end="")
...
if os.path.isfile(dir_script+"/objs.pkl") == False:
    ...
    todo = input_with_prefill(
        "Default file (recommended: 'todo_tdl.md'):", todo)
    todo_open = todo
...
if os.path.isfile(dir+todo_open) == False:
    print("There is no "+todo_open +
          " file. Do you want to create it in directory: "+dir+"? (yes/n)")
...
if len(sys.argv) < 2:  # name of program is 1
    ...
    sep()
    print(cl_header + "file: "+todo_open+" | " +
          str(list(list_of_notes)) + cl_reset)
    if len(lines) < 1:
        sep()
...
# DONE
        ...
        for usr_done in sys.argv[2:]:
            if int(usr_done) > len(lines):
                ...
                print("To do "+cl_number+usr_done +
                      cl_reset+" not found or already done")
            else:
                lines[int(usr_done)] = lines[int(usr_done)
                                             ].replace("- [ ]", "- [X]")
...
# UNDONE
        ...
        for usr_undone in sys.argv[2:]:
            if int(usr_undone) > len(lines):
                print(cl_message + "To do " + cl_reset + cl_number +
                      usr_undone+cl_reset+" does not exist")
            elif lines[int(usr_undone)].find(")[ ]") == 2:
                print(cl_message + "To do " + cl_reset + cl_number +
                      usr_undone+cl_reset+" is in progress")
            else:
                lines[int(usr_undone)] = lines[int(
                    usr_undone)].replace("- [X]", "- [ ]")
                print(cl_message + "To-do " + cl_reset +
                      cl_number + usr_undone + cl_reset + " undone")
# EDIT
        ...
        for usr_edit in sys.argv[2:]:
            if int(usr_edit) > len(lines):
                ...
                lines[int(usr_edit)] = input_with_prefill(
                    ":", lines[int(usr_edit)]).rstrip()+"\n"
# CLEAR
    elif usr_type == "-clear":
...
# SWITCH FILE
        ...
        if switch == "-l":
            ...
            print("There is no "+cl_message + switch+".md"+cl_reset +
                  " file to switch to. Do you want to create it in directory: "+dir+"? (yes/no)")
            usr_create_file = input()
            if usr_create_file in ['y', 'Y', 'yes', 'YES']:
